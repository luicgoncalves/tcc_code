...
#ifndef FINGERPRINT_UPLOADIMAGE
#define FINGERPRINT_UPLOADIMAGE 0x0A //!< Comando para upload da imagem capturada
#endif
...
public:
  ...
  /**
   * @brief  Verifica de modo 1:1 se a digital capturada agora confere com a do ID armazenado
   *         no banco de dados do sensor.
   ...
   * @return uint8_t   Código de retorno: FINGERPRINT_OK (0x00) se combinou, FINGERPRINT_NOMATCH (0x08) se não,
   *                   ou outro erro se ocorreu problema de comunicação ou leitura.
   */
  uint8_t verifyFingerprint1to1(uint16_t id, uint16_t &score);
