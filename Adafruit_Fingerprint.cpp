 ...
 * 
 * @section alteracao feita em 03/03/2025
 * @author Luiz Carlos Goncalves luicgoncalves@gmail.com
 * Inclusao de metodo: uint8_t Adafruit_Fingerprint::verifyFingerprint1to1(uint16_t id, uint16_t &score)
 * Foi incluido um metodo para fazer a validacao 1:1 de um modelo localizado no (id)
 *
 */
...
/**
 * @brief  Verifica de modo 1:1 se a digital recém-capturada confere com a do ID armazenado no sensor.
 * @param  id        ID (PageID) do template salvo no banco a ser comparado
 * @param  score     Referência (out) para retornar o "score" (nível de semelhança)
 * @return uint8_t   Código de retorno do sensor:
 *                   - FINGERPRINT_OK (0x00) se combinou
 *                   - FINGERPRINT_NOMATCH (0x08) se não combinou
 *                   - Outros valores conforme doc (ex. erro de pacote, etc.)
 */
uint8_t Adafruit_Fingerprint::verifyFingerprint1to1(uint16_t id, uint16_t &score) {
  // 0) Opcional: verifique se existe mesmo um template no ID; ou apenas confie que está cadastrado.
  //    Também é comum chamar verifyPassword() no setup se o sensor exige senha.

  //-------------------------------------------------------------------------
  // 1) Carregue o template do ID para CharBuffer2
  //-------------------------------------------------------------------------
  uint8_t p = loadModel(id); // Usa a função interna: envia "LoadChar(0x07, buffer=0x01, id)" 
                             // Por padrão, loadModel carrega no slot=1. Ajuste se quiser no slot=2.
  if (p != FINGERPRINT_OK) {
    Serial.println("Não carregou o modelo");
    return p;  // Se falhar, retorna o código de erro (ex. FINGERPRINT_BADLOCATION)
  } else {
    Serial.println("Modelo carregado");
  }

  //-------------------------------------------------------------------------
  // 2) Peça ao usuário para colocar o dedo no sensor (GenImg + Image2Tz)
  //    e gerar CharBuffer1 ou CharBuffer2.
  //    Usaremos CharBuffer1, assumindo que loadModel usou CharBuffer1 = 0x01, 
  //    então carregaremos a digital nova em CharBuffer2 = 0x02, ou o inverso.
  //    A forma exata depende de como loadModel() foi implementado. 
  //    *Na lib Adafruit, loadModel(id) por padrão faz:
  //          SEND_CMD_PACKET(FINGERPRINT_LOAD, 0x01, highByte(id), lowByte(id));
  //       => buffer #1. Então vamos usar o buffer #2 para a digital nova.
  //-------------------------------------------------------------------------
  // Gera imagem
  uint8_t r;
  do
  ...
  // Converte imagem para CharBuffer2
  uint8_t s;
  do
  {
    s = image2Tz(2); // 2 => slot #2
  } while (s != FINGERPRINT_OK);

  Serial.println("Imagem convertida");
  
  //-------------------------------------------------------------------------
  // 3) Executa a comparação 1:1 => Comando Match (0x03)
  //    Usando a função createModel() não faz sentido, pois ela combina. Precisamos de "Match"
  //-------------------------------------------------------------------------
  GET_CMD_PACKET(FINGERPRINT_MATCH);
  if (packet.data[0] == FINGERPRINT_OK) {
    // OK, pega o score:
    score = ((uint16_t)packet.data[1] << 8) | packet.data[2];
    return FINGERPRINT_OK; // = 0x00
  } else if (packet.data[0] == FINGERPRINT_NOMATCH) {
    // Digitais diferentes
    return FINGERPRINT_NOMATCH; // = 0x08
  } else {
    // Caso de erro de pacote ou outro
    return packet.data[0];
  }
}
